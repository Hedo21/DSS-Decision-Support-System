<?php
include('../config.php');
$sql = "SELECT Id_alternatif FROM alternatif";
$sql1 = "SELECT id_bobot FROM bobot";
$sql2 = "SELECT id_skala FROM skala";
			...
			<div class="row mt-3 justify-content-center">
				<div class="col-md-9">
					<form action="../insert/insertmatrix.php" method="POST">
						<h1 class="form-label">Form Matriks</h1>
						<div class="mb-3">
							<label for="disabledSelect" class="form-label">ID Alternatif</label>
							<select name="idalt" id="disabledSelect" class="form-select">
								<?php foreach ($results as $row) : ?>
									<option><?= $row['Id_alternatif']; ?></option>
								<?php endforeach; ?>
							</select>
						</div>
						<div class="mb-3">
							<label for="disabledSelect" class="form-label">ID Bobot</label>
							<select name="idbobot" id="disabledSelect" class="form-select">
								<?php foreach ($results1 as $row) : ?>
									<option><?= $row['id_bobot']; ?></option>
								<?php endforeach; ?>
							</select>
						</div>
						<div class="mb-3">
							<label for="disabledSelect" class="form-label">ID Skala</label>
							<select name="idskala" id="disabledSelect" class="form-select">
								<?php foreach ($results2 as $row) : ?>
									<option><?= $row['id_skala']; ?></option>
								<?php endforeach; ?>
							</select>
						</div>
